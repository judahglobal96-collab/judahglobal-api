...
exports.getApprovedEventsByOrgUuid = getApprovedEventsByOrgUuid;
exports.getRejectedEvents = getRejectedEvents;
exports.getAdminEventById = getAdminEventById;
...
exports.getEventPaidPromos = getEventPaidPromos;
exports.getAdminSupportLookup = getAdminSupportLookup;
exports.waiveEventPayment = waiveEventPayment;
const db_1 = require("../config/db");
function normalizePgTimezone(timezone) {
    if (!timezone)
        return "UTC";
...
async function getPaidPromoSummary(client, eventId) {
...
async function getPendingEvents(_req, res) {
    try {
...
async function getApprovedEvents(_req, res) {
    try {
        const result = await db_1.db.query(`
      SELECT
        id,
...
async function getApprovedEventsByOrgUuid(req, res) {
    try {
...
async function getRejectedEvents(_req, res) {
    try {
        const result = await db_1.db.query(`
      SELECT
        es.id,
    ...
    try {
      ...
      `, [eventId]);
        ...
        if (!scheduleCheck.start_date ||
            ...
            await logEventAction({
                ...
                errorDetail: "Cannot approve event. Required schedule fields are missing: start_date, start_time, and timezone.",
                metadata: {
                    source: "admin_review",
            ...
            return res.status(400).json({
                success: false,
                message: "Cannot approve event. Required schedule fields are missing: start_date, start_time, and timezone.",
            });
        }
        ...
        if (startDateValue !== null &&
            ...
            await logEventAction({
                ...
                errorDetail: "Cannot approve event. End date cannot be earlier than start date.",
                metadata: {
                    source: "admin_review",
        ...
        if (scheduleCheck.start_date &&
            ...
            await logEventAction({
                ...
                errorDetail: "Cannot approve event. End time cannot be earlier than start time on the same day.",
                metadata: {
                    source: "admin_review",
            ...
            return res.status(400).json({
                success: false,
                message: "Cannot approve event. End time cannot be earlier than start time on the same day.",
            });
        }
        const isOrgEvent = !!eventCheck.org_uuid;
        const isPublicEvent = !isOrgEvent;
        if (isPublicEvent &&
            !["paid", "waived"].includes(String(eventCheck.payment_status || "").toLowerCase())) {
            await client.query("ROLLBACK");
            await logEventAction({
                eventId,
                eventCode: eventCheck.event_code,
        ...
        const resolvedFeaturedFlag = Boolean(eventCheck.featured) || Boolean(promoFlags.has_paid_featured_badge);
      ...
      `, [eventId]);
        ...
        const searchText = [
            ...
            .filter(Boolean)
            .join(" ");
        const startsAtLocal = safeStartDate && normalizedStartTime
            ? `${safeStartDate} ${normalizedStartTime}`
            : null;
        const endsAtLocal = safeEndDate && normalizedEndTime
        ...
        if (!startsAtLocal) {
            ...
            await logEventAction({
                ...
                errorDetail: "Cannot approve event. Unable to compute starts_at_utc because schedule start date/time is incomplete.",
                metadata: {
                    source: "admin_review",
                    safeStartDate,
                    normalizedStartTime,
            ...
            return res.status(400).json({
                success: false,
                message: "Cannot approve event. Unable to compute starts_at_utc because schedule start date/time is incomplete.",
            });
        }
        const finalFeaturedFlag = Boolean(event.featured) || Boolean(event.has_paid_featured_badge);
        await client.query(`
...
async function updateFeaturedStatus(req, res) {
    const client = await db_1.db.connect();
    const eventId = String(req.params.eventId || "");
    const actorUserId = req?.user?.id ?? null;
    const actorRole = req?.user?.role ?? null;
...
async function getEventPaidPromos(req, res) {
    try {
      ...
      LIMIT 1
      `, [eventId]);
        const promoResult = await db_1.db.query(`
      SELECT
        ci.id,
        ci.campaign_id,
        c.campaign_name,
...
async function getAdminSupportLookup(req, res) {
    try {
...
async function waiveEventPayment(req, res) {
    const client = await db_1.db.connect();
    const eventId = String(req.params.eventId || "");
    const actorUserId = req?.user?.id ?? null;
    const actorRole = req?.user?.role ?? null;

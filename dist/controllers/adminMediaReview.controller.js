...
exports.getMediaReviewEventDetail = exports.rejectMediaReview = exports.approveMediaReview = exports.getRejectedMediaReviews = exports.getApprovedMediaReviews = exports.getPendingMediaReviews = void 0;
const db_1 = require("../config/db");
/**
 * GET /api/v1/admin/media-review/pending
 */
const getPendingMediaReviews = async (_req, res) => {
    try {
        const query = `
      SELECT *
      FROM (
...
exports.getPendingMediaReviews = getPendingMediaReviews;
/**
 * GET /api/v1/admin/media-review/approved
 */
const getApprovedMediaReviews = async (_req, res) => {
    try {
        const query = `
      SELECT *
      FROM (
...
exports.getApprovedMediaReviews = getApprovedMediaReviews;
/**
 * GET /api/v1/admin/media-review/rejected
 */
const getRejectedMediaReviews = async (_req, res) => {
    try {
        const query = `
      SELECT *
      FROM (
...
exports.getRejectedMediaReviews = getRejectedMediaReviews;
/**
 * PATCH /api/v1/admin/media-review/:mediaId/approve
 */
const approveMediaReview = async (req, res) => {
    const mediaId = String(req.params.mediaId || "").trim();
    const reviewedByUserId = req?.user?.id || null;
    try {
        await db_1.db.query("BEGIN");
        // 1. Try EVENT MEDIA first
        const eventMediaCheck = await db_1.db.query(`SELECT event_id FROM event_media WHERE id = $1::uuid`, [mediaId]);
        ...
        const promoMediaCheck = await db_1.db.query(`
...
exports.approveMediaReview = approveMediaReview;
/**
 * PATCH /api/v1/admin/media-review/:mediaId/reject
 */
const rejectMediaReview = async (req, res) => {
    const mediaId = String(req.params.mediaId || "").trim();
    const reason = typeof req.body?.reason === "string" ? req.body.reason.trim() : "";
    try {
        await db_1.db.query("BEGIN");
        // 1. Try EVENT MEDIA first
        const eventMediaCheck = await db_1.db.query(`SELECT id FROM event_media WHERE id = $1::uuid`, [mediaId]);
        if (eventMediaCheck.rows.length > 0) {
        ...
        // 2. Try CAMPAIGN PROMO MEDIA
        const promoMediaCheck = await db_1.db.query(`SELECT id FROM campaign_promo_media WHERE id = $1::uuid`, [mediaId]);
        if (promoMediaCheck.rows.length > 0) {
...
exports.rejectMediaReview = rejectMediaReview;
/**
 * GET /api/v1/admin/media-review/event/:eventId
 */
const getMediaReviewEventDetail = async (req, res) => {
    ...
    `;
        const [eventResult, mediaResult] = await Promise.all([
            db_1.db.query(eventQuery, [eventId]),
            db_1.db.query(mediaQuery, [eventId]),
        ]);
...
exports.getMediaReviewEventDetail = getMediaReviewEventDetail;

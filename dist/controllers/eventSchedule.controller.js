...
exports.saveEventSchedule = saveEventSchedule;
...
function normalizeScheduleType(payload) {
    return (payload.schedule_type || payload.event_type || "one_time");
}
...
function buildRRule(args) {
    const { scheduleType, interval, recurrenceDays, monthlyMode, monthlyDay, monthlyWeek, monthlyWeekday, recurrenceUntil, } = args;
...
function validateSchedule(payload) {
    ...
    const monthlyDay = payload.monthly_day === null || payload.monthly_day === undefined
        ? null
        : Number(payload.monthly_day);
    const monthlyWeek = payload.monthly_week === null || payload.monthly_week === undefined
        ? null
...
async function saveEventSchedule(req, res) {

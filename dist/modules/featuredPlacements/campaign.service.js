...
exports.getPlacementPriceCents = getPlacementPriceCents;
exports.checkCampaignAvailability = checkCampaignAvailability;
exports.getCampaignCalendarAvailability = getCampaignCalendarAvailability;
exports.reserveCampaign = reserveCampaign;
exports.createCampaignReview = createCampaignReview;
exports.createCampaignCheckoutSession = createCampaignCheckoutSession;
exports.uploadCampaignPromoMedia = uploadCampaignPromoMedia;
exports.getCampaignPromoMediaStatus = getCampaignPromoMediaStatus;
exports.getCampaignPaymentSuccessBySessionId = getCampaignPaymentSuccessBySessionId;
...
// ================================
function shouldWaiveEventFee({ waiveEventPayment, }) {
    if (waiveEventPayment)
        return true;
    return false;
}
function getPlacementPrice({ type, isOrgFlow, orgSubscriptionActive, regionCode, }) {
...
function getPlacementPriceCents(type, options) {
...
function toWeekStartUtc(dateString) {
...
function toUtcDayRange(dateString) {
...
async function findWeeklyPlacementAvailability(client, placementType, placementDate, regionCode) {
    const normalizedType = normalizePlacementType(placementType);
    const { startUtc, endUtc } = toUtcDayRange(placementDate);
    const normalizedRegionCode = normalizeRegionCode(regionCode);
    const capacity = normalizedType === "hero"
        ? 2
...
async function getLinkedEventById(client, eventId) {
    if (!eventId)
        return null;
    const result = await client.query(`
    SELECT
...
async function getOrgSubscriptionStatus(client, orgUuid) {
    if (!orgUuid)
        return false;
    const result = await client.query(`
    SELECT
...
async function getCampaignById(client, campaignId) {
    const result = await client.query(`
    SELECT
      id,
      campaign_name,
...
async function getCampaignItemByPlacement(client, campaignId, placementType, campaignItemId) {
...
async function checkCampaignAvailability(items) {
    const results = [];
    for (const item of items) {
        const placementType = normalizePlacementType(item.placementType);
        const placementDate = placementType === "event_fee" || placementType === "org_subscription"
            ? item.placementDate
        ...
        const clientKey = `${placementType}|${placementDate}|${quantity}|${durationDays ?? "none"}`;
        if (placementType === "event_fee" ||
        ...
        let firstUnavailableWeekStartDate = null;
        for (let weekOffset = 0; weekOffset < quantity; weekOffset += 1) {
            const weekStartDate = addUtcDays(placementDate, weekOffset * 7);
            const availabilityRow = await findWeeklyPlacementAvailability(db_1.db, placementType, weekStartDate, item.regionCode);
            const availableCount = Math.max(0, availabilityRow.capacity - availabilityRow.sold_count);
            if (availableCount < 1) {
...
async function getCampaignCalendarAvailability(params) {
    ...
    const calendarWeeks = [];
    if (placementType === "org_subscription" ||
    ...
    for (let weekOffset = 0; weekOffset < weeks; weekOffset += 1) {
        const weekStartDate = addUtcDays(startDate, weekOffset * 7);
        const availabilityRow = await findWeeklyPlacementAvailability(db_1.db, placementType, weekStartDate, regionCode);
        const availableCount = Math.max(0, availabilityRow.capacity - availabilityRow.sold_count);
...
async function reserveCampaign(input) {
    ...
    try {
      ...
      `, [
        ...
        const campaignItems = [];
        const normalizedInputItems = [...input.items];
        if (shouldAutoIncludeEventFee({
        ...
        for (const rawItem of normalizedInputItems) {
            ...
            const quantity = placementType === "event_fee" ||
            ...
            const durationDays = rawItem.durationDays === null || rawItem.durationDays === undefined
                ? getDefaultDurationDays(placementType)
                : Number(rawItem.durationDays);
            const normalizedPlacementDate = placementType === "event_fee" ||
                placementType === "org_subscription" ||
                isEventLifecyclePlacement(placementType) ||
                isDurationPlacement(placementType)
                ...
                : isWeeklyPlacement(placementType)
                    ...
                    : rawItem.placementDate;
            const placementDateValue = new Date(`${normalizedPlacementDate}T00:00:00.000Z`);
...
async function createCampaignReview(campaignId) {
    ...
    const linkedEvent = row.linked_event_id
        ? await getLinkedEventById(db_1.db, row.linked_event_id)
        : null;
    const orgSubscriptionActive = await getOrgSubscriptionStatus(db_1.db, row.org_uuid || linkedEvent?.org_uuid || null);
    return {
        ...row,
        org_subscription_active: orgSubscriptionActive,
        waive_event_payment: Boolean(linkedEvent?.waive_event_payment),
        items: Array.isArray(row.items) && row.items.length === 1 && row.items[0] === null
            ? []
            : row.items || [],
    };
}
async function createCampaignCheckoutSession(campaignId) {
    const client = await db_1.db.connect();
    try {
        await client.query("BEGIN");
      ...
      `, [campaignId]);
        ...
        const linkedEvent = campaign.linked_event_id
            ...
            : null;
        const orgSubscriptionActive = await getOrgSubscriptionStatus(client, campaign.org_uuid || linkedEvent?.org_uuid || null);
      ...
      `, [campaignId]);
        ...
        for (const item of items) {
            const placementType = normalizePlacementType(item.placement_type);
            ...
            for (let weekOffset = 0; weekOffset < quantity; weekOffset += 1) {
                ...
                const availabilityRow = await findWeeklyPlacementAvailability(db_1.db, placementType, weekStartDate, item.region_code);
                const availableCount = Math.max(0, availabilityRow.capacity - availabilityRow.sold_count);
                if (availableCount < 1) {
                    throw new Error(`${getPlacementLabel(placementType)} is no longer available for the week of ${weekStartDate}.`);
                }
            }
        }
            ...
            return {
                price_data: {
                    ...
                    product_data: {
                        ...
                        description: placementType === "event_fee"
                ...
                quantity: placementType === "event_fee" ||
                    placementType === "official_flyer" ||
                    placementType === "featured_badge" ||
                    placementType === "org_subscription" ||
                    isDuration
...
async function uploadCampaignPromoMedia(input) {
    const client = await db_1.db.connect();
    try {
        await client.query("BEGIN");
        ...
        const campaignItem = await getCampaignItemByPlacement(client, input.campaignId, input.placementType, input.campaignItemId || null);
        if (!campaignItem) {
...
async function getCampaignPromoMediaStatus(input) {
    const client = await db_1.db.connect();
    try {
        const campaign = await getCampaignById(client, input.campaignId);
        if (!campaign) {
      ...
      `, [campaignItem.id]);
        ...
        const moderationStatus = pendingMedia?.moderation_status ||
        ...
        return {
            ...
            mediaStatus: {
                ...
                file_name: pendingMedia?.file_name ||
                    activeMedia?.file_name ||
                    latestMedia?.file_name ||
                    null,
                last_uploaded_at: pendingMedia?.created_at ||
...
async function getCampaignPaymentSuccessBySessionId(sessionId) {
    const session = await stripe.checkout.sessions.retrieve(sessionId);
    const campaignId = String(session.metadata?.campaign_id ||

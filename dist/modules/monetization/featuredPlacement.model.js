...
exports.getAvailableFeaturedPlacements = getAvailableFeaturedPlacements;
exports.getFeaturedPlacementInventoryById = getFeaturedPlacementInventoryById;
exports.getFeaturedPlacementInventoryByIds = getFeaturedPlacementInventoryByIds;
exports.isPlacementInventoryAvailable = isPlacementInventoryAvailable;
exports.getExistingPlacementReservationForEvent = getExistingPlacementReservationForEvent;
exports.reserveFeaturedPlacement = reserveFeaturedPlacement;
exports.getCampaignAvailability = getCampaignAvailability;
exports.autoAssignAndReserveCampaign = autoAssignAndReserveCampaign;
const db_1 = require("../../config/db");
function mapPromoTypeToInventoryFilter(promoType) {
    switch (promoType) {
        case "homepage":
            return {
                pageName: "homepage",
...
function buildUtcWindow(startDate, durationDays) {
...
async function getAvailableFeaturedPlacements(pageName) {
    const values = [];
    let pageFilterSql = "";
    if (pageName) {
...
async function getFeaturedPlacementInventoryById(inventoryId) {
...
async function getFeaturedPlacementInventoryByIds(inventoryIds) {
...
async function isPlacementInventoryAvailable(inventoryId) {
    const result = await db_1.db.query(`
    SELECT fpi.id
    FROM featured_placement_inventory fpi
    WHERE fpi.id = $1
...
async function getExistingPlacementReservationForEvent(eventId) {
    const result = await db_1.db.query(`
    SELECT
      efp.id,
      efp.event_id,
...
async function reserveFeaturedPlacement({ eventId, inventoryId, }) {
...
async function getCampaignAvailability({ promoType, startDate, durationDays, }) {
...
async function autoAssignAndReserveCampaign({ eventId, promoType, startDate, durationDays, }) {
    const filter = mapPromoTypeToInventoryFilter(promoType);
    if (!filter) {
        await db_1.db.query(`
      UPDATE event_submissions

...
exports.findEventSubmissionForPayment = findEventSubmissionForPayment;
...
async function findEventSubmissionForPayment(eventId) {
    const query = `
  SELECT
    es.id,
    es.owner_user_id,
  ...
  LIMIT 1
`;
    const result = await db_1.db.query(query, [eventId]);
    return result.rows[0] || null;
}
async function createPendingPayment(params) {
    const { eventId, amountCents, currency, checkoutSessionId, customerEmail = null, paymentProvider = 'stripe', } = params;
...
async function markPaymentPaid(params) {
    const { checkoutSessionId, paymentIntentId = null, customerEmail = null, } = params;
    const client = await db_1.db.connect();
    try {
        await client.query('BEGIN');
...
async function markPaymentFailed(checkoutSessionId) {
    const client = await db_1.db.connect();
    try {
        await client.query('BEGIN');
        const paymentLookup = await client.query(`

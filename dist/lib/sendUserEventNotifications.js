...
exports.sendUserEventNotifications = sendUserEventNotifications;
const nodemailer_1 = __importDefault(require("nodemailer"));
const db_1 = require("../config/db");
const newEventsEmailTemplate_1 = require("../lib/emailTemplates/newEventsEmailTemplate");
const APP_URL = process.env.APP_URL || "http://localhost:5173";
const FROM_EMAIL = process.env.NOTIFICATION_FROM_EMAIL || "Judah Global <no-reply@judahglobal.com>";
...
function buildEventCard(event) {
    const badge = event.is_major_event
        ...
        : event.is_featured
            ? "⭐ FEATURED"
            : "JUDAH GLOBAL EVENT";
    const imageUrl = event.media_url ||
...
function buildEmailHtml(user, events) {
...
async function sendUserEventNotifications() {
    ...
    for (const user of users) {
        try {
            ...
            const html = (0, newEventsEmailTemplate_1.buildNewEventsEmail)(events.map((e) => ({
            ...
            await sendEmail(user.email, "New Events Posted Near You | Judah Global", html);
            sentCount += 1;
        }
        catch (error) {
            failedCount += 1;
            console.error(`Failed to send notification email to user ${user.user_id}:`, error);
        }
    }
    return {
        sentCount,

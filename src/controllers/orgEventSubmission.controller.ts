...
import { createEventVerification } from "../services/eventVerification.service";
import { isValidSponsorType } from "../lib/event-options";

export async function submitEventForReview(req: Request, res: Response) {
  ...
  try {
    ...
    if (sponsor?.sponsorType && !isValidSponsorType(sponsor.sponsorType)) {
    ...
    const ownerUserId = (req as any)?.user?.id || null;
    ...
    if (existingVerification.rows.length === 0) {
      verification = await createEventVerification(event_id);
    } else {

...
function formatWeekly(days: WeekdayCode[]) {
  return days.map((d) => weekdayLabels[d] || d).join(", ");
}

function formatMonthlyWeek(week: number | null) {
...
function buildRecurrenceSummary(schedule: any): string | null {
  ...
  if (schedule.schedule_type === "recurring_monthly") {
    ...
    if (schedule.monthly_mode === "nth_weekday") {
      return `Repeats every ${every} month(s) on ${formatMonthlyWeek(
        schedule.monthly_week
      )} ${weekdayLabels[schedule.monthly_weekday as WeekdayCode]}${until}`;
    }
...
export async function getEventReview(req: Request, res: Response) {

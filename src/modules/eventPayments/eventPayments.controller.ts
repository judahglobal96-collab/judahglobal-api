...
import {
  ...
  findEventSubmissionForPayment,
  markPaymentPaid,
} from "./eventPayments.model";
...
const PRICING_CENTS: Record<NormalizedPlacementType, number> = {
  hero: 44900,
  homepage_top: 24900,
  discovery_top: 24900,
  featured_badge: 7900,
...
function normalizePlacementType(type: string): NormalizedPlacementType {
  switch (type) {
    case "homepage_top":
...
function normalizePromoPlacements(value: unknown): PromoPlacementSelection[] {
  if (!Array.isArray(value)) return [];

  return value
    .map((raw) => {
      const item = raw as Partial<PromoPlacementSelection>;
      ...
      if (!placementType || !startDate) return null;
    ...
    .filter(Boolean) as PromoPlacementSelection[];
}

function buildPricingSummary(placements: PromoPlacementSelection[]) {
...
export async function createOrgSubscriptionCheckoutController(
  req: Request,
  res: Response
) {
  try {
...
export async function createCheckoutSessionController(
  ...
  try {
    ...
    const event = await findEventSubmissionForPayment(String(eventId));
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (paymentType === "org_subscription") {
        const orgAccountId =
          session.metadata?.orgAccountId || session.client_reference_id;
        ...
        const eventId =
          session.metadata?.eventId || session.client_reference_id;

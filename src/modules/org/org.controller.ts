...
async function verifyOrgOwner(orgUuid: string, userId: string) {
  ...
  if (!org) return { ok: false, status: 404, message: "Organization not found", org: null };
...
export async function getOrgEventMetadataForEdit(
  req: AuthenticatedRequest,
  res: Response
) {
  try {
...
export async function updateOrgEventMetadata(
  req: AuthenticatedRequest,
  res: Response
) {
...
export async function getOrgApprovedEvents(
  req: AuthenticatedRequest,
  res: Response
) {
  try {

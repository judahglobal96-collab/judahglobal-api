...
import { convertHoldSessionToReservations } from "./services/placementReservation.service";
import { syncCampaignPromotionFlags } from "../../services/promotionSync.service";

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, {
  apiVersion: "2026-02-25.clover",
});

type LogEventActionParams = {
  eventId: string;
...
async function getEventCode(eventId: string): Promise<string | null> {
  try {
...
function resolveEventIdFromCheckoutSession(
...
function resolveOrgAccountIdFromCheckoutSession(
  session: Stripe.Checkout.Session
): string | null {
  return session.metadata?.orgAccountId || null;
}

function resolveOrgUuidFromCheckoutSession(
  session: Stripe.Checkout.Session
): string | null {
  return session.metadata?.orgUuid || null;
}

function isPlacementPromoPurchase(session: Stripe.Checkout.Session): boolean {
...
async function resolveLinkedEventIdForCampaign(
  campaignId: string
): Promise<string | null> {
  try {
    const result = await db.query(
...
async function markCampaignPaid(params: {
  ...
  try {
    ...
    if (linkedEventId) {
      ...
      const placementTypes = placementResult.rows.map(
        (row) => row.placement_type
      );
    ...
    await syncCampaignPromotionFlags(campaignId);
    await db.query("COMMIT");
  } catch (error) {
    await db.query("ROLLBACK");
    throw error;
  ...
  try {
    switch (stripeEvent.type) {
      case "checkout.session.completed": {
        ...
        const placementHoldSessionId =
          resolvePlacementHoldSessionIdFromCheckoutSession(session);
        ...
        if (purchaseType === "campaign" && campaignId) {
          try {
            ...
            if (eventId) {
              ...
              console.warn(
                "Campaign payment succeeded but no linked event could be resolved.",
                {
                  campaignId,
                  sessionId: session.id,
                }
        ...
        if (placementPromo) {
          if (!placementHoldSessionId) {
            console.warn(
              "Placement promo checkout.session.completed received without placement hold session metadata"
            );
          ...
          try {
            await convertHoldSessionToReservations({
        ...
        if (!eventId) {
          console.warn(
            "Stripe checkout.session.completed received without eventId metadata/client_reference_id"
          );
      ...
      case "payment_intent.payment_failed": {
        ...
        if (!eventId) {
          console.warn(
            "Stripe payment_intent.payment_failed received without eventId metadata"
          );
      ...
      case "checkout.session.expired": {
        ...
        if (!eventId) {
          console.warn(
            "Stripe checkout.session.expired received without eventId metadata/client_reference_id"
          );
        ...
        try {
          ...
          console.error(
            "Failed to update event_submissions for expired checkout:",
            updateError
          );
        }
